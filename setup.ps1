...
#   - Go 1.21+  (para el seed del Proyecto 1)
...
function Get-Python {
    ...
    Write-Fail "Python no encontrado en PATH. Instalá Python 3 desde https://python.org"
}
...
function Wait-For($description, $maxTries, $sleepSec, $scriptBlock) {
    for ($i = 1; $i -le $maxTries; $i++) {
        try {
            $result = & $scriptBlock
            if ($result) { Write-Ok "$description lista"; return }
        } catch { }
        Write-Step "... esperando $description ($i/$maxTries)"
        Start-Sleep -Seconds $sleepSec
    }
    Write-Fail "$description no respondió después de $($maxTries * $sleepSec)s"
}

# ── Docker Compose wrapper ────────────────────────────────────────────────────
$COMPOSE_CMD = "docker", "compose", "-f", "deployments/docker-compose.yml", "--env-file", ".env"

function Invoke-Compose {
    param([string[]]$args)
    & docker compose -f deployments/docker-compose.yml --env-file .env @args
}
...
$P1 = Resolve-Path $P1 -ErrorAction SilentlyContinue
if (-not $P1) { Write-Fail "No se encontró el Proyecto 1 en '$P1'. Usá -P1 para indicar la ruta." }
$PYTHON = Get-Python

# ── [1/9] Bajar Proyecto 2 ───────────────────────────────────────────────────
Write-Header "▶ [1/9] Bajando Proyecto 2 (con volúmenes)..."
try { Invoke-Compose @("down", "-v") } catch { Write-Warn "Nada que bajar en P2" }

# ── [2/9] Bajar Proyecto 1 ───────────────────────────────────────────────────
Write-Header "▶ [2/9] Bajando Proyecto 1 (con volúmenes)..."
try {
    Push-Location $P1
...
Write-Step "Esperando que la API del Proyecto 1 esté lista..."
...
if ($LASTEXITCODE -ne 0) { Pop-Location; Write-Fail "Seed base falló" }
Pop-Location
Write-Ok "Seed base completado"

# ── [5/9] Seed transacciones ─────────────────────────────────────────────────
Write-Header "▶ [5/9] Sembrando transacciones (órdenes + reservaciones)..."
& $PYTHON -m pip install psycopg2-binary --quiet 2>$null
& $PYTHON scripts/seed_transactions.py --orders 300 --reservations 150
if ($LASTEXITCODE -ne 0) { Write-Fail "seed_transactions.py falló" }
Write-Ok "Transacciones sembradas"

# ── [6/9] Configurar .env y levantar stack de analítica ──────────────────────
Write-Header "▶ [6/9] Configurando .env y levantando stack de analítica..."
& $PYTHON -m pip install cryptography --quiet 2>$null
& $PYTHON scripts/configure_env.py
if ($LASTEXITCODE -ne 0) { Write-Fail "configure_env.py falló" }

Invoke-Compose @("up", "--build", "-d")

Write-Step "Esperando que analytics-db esté lista..."
Wait-For "analytics-db" 20 5 {
    $out = docker exec ra_analytics_db pg_isready -U analytics 2>&1
...
Write-Step "Esperando que Airflow DB esté lista..."
...
foreach ($conn in $connections) {
    try {
        ...
        Write-Ok "Conexión '$($conn.id)' creada"
    } catch {
        Write-Warn "Conexión '$($conn.id)' ya existe"
    }
}
...
Write-Step "Esperando que Airflow esté healthy..."
...
Write-Step "ETL en ejecución — podés monitorear en http://localhost:8085"

Write-Step "Esperando que el DAG termine (máx 20 min)..."
$dagDone = $false
for ($i = 1; $i -le 80; $i++) {
    $state = docker exec ra_airflow airflow dags list-runs -d restaurants_etl --output plain 2>$null |
    ...
    if ($state -eq "success") {
        ...
        Write-Fail "DAG falló — revisá http://localhost:8085"
    }
    Write-Step "... DAG estado: $($state ?? 'running') ($i/80)"
    Start-Sleep -Seconds 15
}
if (-not $dagDone) { Write-Warn "Timeout esperando el DAG — revisá el estado en Airflow UI" }
...
docker exec ra_hive_server beeline -u "jdbc:hive2://localhost:10000" `
    ...
    2>&1 | Select-String -NotMatch "SLF4J"
...
if ($LASTEXITCODE -ne 0) { Write-Fail "Spark analytics job falló" }
...
if ($LASTEXITCODE -ne 0) { Write-Fail "Carga del grafo Neo4J falló" }
Write-Ok "Grafo cargado"

Write-Step "Configurando dashboards de Metabase..."
& $PYTHON dashboards/metabase/setup_metabase.py
if ($LASTEXITCODE -ne 0) { Write-Warn "setup_metabase.py tuvo errores — revisá manualmente" }
